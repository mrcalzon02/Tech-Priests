...
-- Tech Priests 0.1.527
--
-- Universal physical logistics fetch executor.  If the scheduler/dispatcher
-- wants an item and the station catalog already knows a container, machine
-- inventory, vehicle trunk, corpse inventory, or loose ground stack that holds
-- that item, the priest must physically go fetch it before raw mining,
...
local function requested_count_from_order(o, item)
  ...
  for _, k in ipairs({"missing_count","needed_count","need_count","required_count","target_count","requested_count","amount","quantity","count"}) do
    local n = tonumber(o[k])
    if n and n > 0 then return n end
  end
  if type(o.request) == "table" then
    for _, k in ipairs({"missing_count","needed_count","need_count","required_count","target_count","requested_count","amount","quantity","count"}) do
      local n = tonumber(o.request[k])
      if n and n > 0 then return n end
    end
  end
  return nil
...
local function active_request(pair)
  ...
  for _, field in ipairs({"active_supply_request","logistic_requested_item","scavenge","inventory_scan"}) do
...
local function deposit_to_station(pair, item, count)
  ...
  if type(_G.tech_priests_safe_deposit_item) == "function" then
    local ok, did, why = pcall(_G.tech_priests_safe_deposit_item, pair, item, count, "logistics-fetch-0527")
    if ok and did then return count end
  end
  if type(_G.tech_priests_0358_try_deposit_to_station) == "function" then
    local ok, inserted = pcall(_G.tech_priests_0358_try_deposit_to_station, pair, item, count, "logistics-fetch-0527")
...
local function catalog_storage_source(pair, item)
  ...
  -- Direct catalog fallback to prefer the closest concrete inventory instance.
  local cat = nil
...
local function loose_ground_source(pair, item)
  if not (valid_pair(pair) and item) then return nil end
  local r = tonumber(pair.radius) or (_G.get_station_operating_radius and select(2,pcall(_G.get_station_operating_radius,pair.station))) or 24
  r = math.max(8, tonumber(r) or 24)
  local p = pair.station.position
  local area = {{p.x-r,p.y-r},{p.x+r,p.y+r}}
  local ents = routed_find(pair.station.surface, { area=area, type="item-entity", limit=128 }, "pickup", "pickup:" .. tostring(pair.station.surface.index) .. ":" .. tostring(pair.station.force.index) .. ":" .. tostring(station_unit(pair) or "?") .. ":" .. tostring(item), 60 * 4)
  if not ents then return nil end
  local best, bd = nil, nil
  for _, e in pairs(ents) do
    if valid(e) and e.stack and e.stack.valid_for_read and e.stack.name == item then
...
local function request_move(pair, source, item)
  ...
  if _G.tech_priests_request_movement_0418 then
    local ok, res = pcall(_G.tech_priests_request_movement_0418, pair, pos, "known-source-fetch-0527", { owner="logistics-fetch-0527", priority=M.fetch_priority, ttl=M.fetch_ttl, radius=1.15, distraction=defines and defines.distraction and defines.distraction.none or nil })
    if ok and res ~= false then
      pair.mode = "moving-to-known-source"
      pair.logistics_fetch_0527 = { phase="moving-to-source", item=item, source=source, source_unit=source.unit_number, source_name=source.name, x=pos.x, y=pos.y, tick=now() }
      pair.logistics_fetch_0526 = pair.logistics_fetch_0527
      record(pair, "move-to-known-source", tostring(item) .. " from " .. tostring(source.name) .. "#" .. tostring(source.unit_number or "?"))
      return true
    end
  end
  return false
...
function M.service_pair(pair, reason)
  ...
  local req = active_request(pair)
  local item = req and req.item or nil
  ...
  if src.kind == "loose-ground-item-0527" then
    ...
    if left <= 0 then
      pcall(function() src.source.destroy() end)
    else
      pcall(function() stack.count = left end)
    end
  else
  ...
  if inserted < removed then
    if inv and inv.valid then pcall(function() inv.insert({ name=item, count=removed-inserted }) end)
    else pcall(function() pair.station.surface.spill_item_stack{position=pair.priest.position, stack={name=item,count=removed-inserted}, force=pair.station.force, allow_belts=false} end) end
  end
  pair.logistics_fetch_0527 = { phase="deposited", item=item, count=inserted, requested=need_total, remaining_after=math.max(0, need_total - station_count(pair,item)), source_name=src.source.name, source_unit=src.source.unit_number, source_kind=src.kind, tick=now() }
  pair.logistics_fetch_0526 = pair.logistics_fetch_0527
  if inserted > 0 then
    pair.scavenge = nil
    pair.inventory_scan = nil
    pair.logistic_requested_item = nil
    record(pair, "fetched-known-source", tostring(item) .. " x" .. tostring(inserted) .. " from " .. tostring(src.source.name) .. "#" .. tostring(src.source.unit_number or "?") .. " need=" .. tostring(need_total) .. " source=" .. tostring(req.source or "?"))
  ...
  D.service_pair = function(pair, reason, ...)
    ...
    if r.enabled ~= false and r.dispatcher_priority_fetch ~= false and valid_pair(pair) then
      ...
      if acted then
        ...
        pair.dispatcher_0510.reason = tostring(why or "known-source-fetch-0527")
        pair.dispatcher_0510.acted = true
        pair.dispatcher_0510.result = tostring(why or "known-source-fetch-0527")
...
function M.describe_pair(pair)
  ...
  local src = item and known_fetch_source(pair, item) or nil
  local fetch = pair.logistics_fetch_0527 or pair.logistics_fetch_0526 or {}
...
local function install_command()
  ...
  pcall(function() commands.remove_command("tp-logistics-fetch-0527") end)
  commands.add_command("tp-logistics-fetch-0527", "Tech Priests 0.1.527: physical known-resource/source fetch diagnostics. Params: all/on/off", function(event)
    ...
    local msg = "[tp-logistics-fetch-0527] enabled=" .. tostring(r.enabled) .. " fetched=" .. safe(r.stats["fetched-known-source"] or 0) .. " moves=" .. safe(r.stats["move-to-known-source"] or 0)
...
local function wrap_diagnostics()
  ...
  diag.logistics_fetch_0527_wrapped = true
  diag.pair_dump_lines = function(...)
    ...
    lines[#lines+1] = "PAIR-DUMP-0468 LOGISTICS-FETCH-0527 END"
    return lines
  end
  return true
end
