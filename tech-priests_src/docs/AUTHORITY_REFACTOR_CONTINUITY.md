...
`station_pair_recovery_0363` is retired. `station_pair_state_0362` remains the sole pair-ledger owner; canonical pair creation and `0499` recovery refresh that ledger directly. Reverse-map truth, missing-priest recovery, migration integrity, and inventory custody remain with `0499`/`0503`, `0734`, and the canonical inventory owners.
