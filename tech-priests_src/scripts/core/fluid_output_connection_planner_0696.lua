...
-- sink. Sink identity, input capability, fluid identity, and free capacity are
-- revalidated before every step. A free sink interface is mandatory when the plan
-- is created; after the first route pipe consumes that interface, subsequent
-- validation correctly accepts the attached route while continuing all other
-- checks. The existing construction planner remains sole placement authority.
...
local function pos(value) if not value then return nil end local x=tonumber(value.x or value[1]); local y=tonumber(value.y or value[2]); if not (x and y) then return nil end return {x=math.floor(x+0.5),y=math.floor(y+0.5)} end
...
local function root()
  ...
  local r=storage.tech_priests[M.storage_key] or {version=M.version,enabled=true,ordinary_pipe_only=true,compatible_sink_required=true,stats={},recent={},plans={}}
...
local function reservations() local r=rawget(_G,"TechPriestsWorkReservations0601"); if r then return r end local ok,module=pcall(require,"scripts.core.work_reservations"); return ok and module or nil end
local function doctrine() local d=rawget(_G,"TechPriestsFluidNetworkDoctrine0689"); if d then return d end local ok,module=pcall(require,"scripts.core.fluid_network_doctrine_0689"); return ok and module or nil end
...
local function accepts_input(box,index)
  local saw=false
  for _,prototype in ipairs(prototype_records(box,index)) do local production; pcall(function() production=prototype.production_type end); production=lower(production); if production~="" then saw=true end; if production=="input" or production=="input-output" then return true end end
  return not saw
end
...
local function segment_contents(box,index) local contents={}; if box and box.valid and index then pcall(function() contents=box.get_fluid_segment_contents(index) or {} end) end; return type(contents)=="table" and contents or {} end
local function local_fluid(box,index) local fluid; if box and box.valid then pcall(function() fluid=box[index] end) end; if type(fluid)=="table" and fluid.name and (tonumber(fluid.amount) or 0)>0 then return {name=fluid.name,amount=tonumber(fluid.amount) or 0} end return nil end
...
local function segment_state(entity,index,fluid)
  ...
  local contents=segment_contents(box,index); local current=local_fluid(box,index); if next(contents)==nil and current then contents[current.name]=current.amount end
  ...
  local filter,filter_source=filter_name(box,index); local occupied=sum_contents(contents); local capacity=segment_capacity(box,index)
  return {box=box,contents=contents,same=same,wrong=wrong,filter=filter,filter_source=filter_source,occupied=occupied,capacity=capacity,free=math.max(0,capacity-occupied),segment_id=segment_id(box,index)},"ok"
end
...
local function pipe_connections(entity,index)
  ...
  for _,connection in pairs(connections or {}) do if type(connection)=="table" then local owner; if connection.target then pcall(function() owner=connection.target.owner end) end; local position=connection.target_position or connection.position; out[#out+1]={position=position and {x=position.x,y=position.y} or nil,connected=connection.target~=nil or valid(owner)} end end
  return out
end
local function free_interfaces(entity,index) local out={}; for _,connection in ipairs(pipe_connections(entity,index)) do if not connection.connected and connection.position then out[#out+1]=connection.position end end; return out end

local function revalidate_sink(proposal,require_free_interface)
  ...
  local state=segment_state(sink.entity,sink.fluidbox_index,proposal.fluid); if not state then return false,"sink-no-fluidbox" end
  ...
  if require_free_interface~=false and #interfaces==0 then return false,"sink-interface-unavailable" end
  sink.current_amount=state.same; sink.occupied=state.occupied; sink.capacity=state.capacity; sink.free_capacity=state.free; sink.segment_id=state.segment_id
  if #interfaces>0 then sink.interfaces=interfaces end
  return true,"sink-valid",state
end

local function machine_output_segment(proposal) if not (proposal and valid(proposal.machine) and proposal.output_fluidbox_index) then return nil end local box=fluidbox(proposal.machine); return box and segment_id(box,proposal.output_fluidbox_index) or nil end
local function territory_allowed(pair,position) local c=constraints(); if c and type(c.interior_position_allowed)=="function" then local ok,allowed=pcall(c.interior_position_allowed,pair,position,2.5); return ok and allowed==true end local radius=tonumber(pair.radius) or 28; return dist_sq(pair.station.position,position)<=math.max(8,radius-2.5)^2 end
local function entities_at(surface,position) local entities={}; pcall(function() entities=surface.find_entities_filtered({area={{position.x-0.35,position.y-0.35},{position.x+0.35,position.y+0.35}}}) or {} end); return entities end
...
local function existing_route_pipe(pair,position,plan)
  for _,entity in pairs(entities_at(pair.station.surface,position)) do
    if valid(entity) and entity.force==pair.station.force and (entity.type=="pipe" or entity.type=="pipe-to-ground") then
      ...
      if box then for index=1,#box do local state=segment_state(entity,index,plan.fluid); if state and not state.wrong then local typed=state.filter==plan.fluid; local same=state.same>0.001; local allowed_segment=state.segment_id and (state.segment_id==plan.sink_segment_id or state.segment_id==plan.output_segment_id); if same or typed or allowed_segment then return entity end end end end
    end
  end
  return nil
end
...
local function adjacent_safe(pair,position,plan)
  ...
  for _,entity in pairs(entities) do local box=fluidbox(entity); if box then for index=1,#box do local state=segment_state(entity,index,plan.fluid); if state then if state.wrong then return false,"adjacent-wrong-fluid:"..state.wrong end; local same=state.same>0.001; local typed=state.filter==plan.fluid; local allowed_segment=state.segment_id and (state.segment_id==plan.sink_segment_id or state.segment_id==plan.output_segment_id); local endpoint=entity==plan.machine or entity==plan.sink.entity; if not (same or typed or allowed_segment or endpoint) then return false,"adjacent-empty-unfiltered:"..safe(entity.name) end end end end end
...
local function bfs(pair,start_position,goal_position,plan)
  ...
  local queue={start}; local head=1; local start_key=pos_key(start); local goal_key=pos_key(goal); local visited={[start_key]=true}; local came={}; local positions={[start_key]=start}; local nodes=0; local directions={{1,0},{-1,0},{0,1},{0,-1}}
  while head<=#queue and nodes<M.max_search_nodes do
    ...
    if current_key==goal_key then local route=reconstruct(came,current_key,positions); if #route>M.max_route_tiles then return nil,"route-too-long:"..tostring(#route) end; stat("route_nodes_visited",nodes); return route,"route-found" end
    for _,direction in ipairs(directions) do local next_position={x=current.x+direction[1],y=current.y+direction[2]}; local key=pos_key(next_position); if not visited[key] then local allowed=can_route_tile(pair,next_position,plan); if allowed then visited[key]=true; came[key]=current_key; positions[key]=next_position; queue[#queue+1]=next_position end end end
  end
  stat("route_search_exhausted"); return nil,nodes>=M.max_search_nodes and "search-budget-exhausted" or "no-route"
end
local function best_route(pair,proposal,plan)
  ...
  local best,best_why
  for _,machine_target in ipairs(machine_targets) do for _,sink_target in ipairs(sink_targets) do local route,why=bfs(pair,machine_target,sink_target,plan); if route and (not best or #route<#best) then best,best_why=route,why end end end
  return best,best_why or "no-compatible-output-route"
end
local function new_tiles(pair,route,plan) local tiles={}; for _,position in ipairs(route or {}) do if not existing_route_pipe(pair,position,plan) then tiles[#tiles+1]={x=position.x,y=position.y} end end local reversed={}; for i=#tiles,1,-1 do reversed[#reversed+1]=tiles[i] end; return reversed end
...
local function claim_route(pair,plan)
  ...
  local claimed={}
  for _,position in ipairs(plan.tiles or {}) do local target={surface_index=pair.station.surface.index,position={x=position.x,y=position.y}}; local ok,why=r.claim("construction",target,pair,M.reservation_ttl,{surface_index=pair.station.surface.index,force_index=pair.station.force.index,fluid=plan.fluid,plan_id=plan.id,source="fluid-output-connection-planner-0696"}); if not ok then for _,old in ipairs(claimed) do pcall(r.release,"construction",old,pair) end; stat("route_reservation_denied"); return false,why or "reservation-denied" end; claimed[#claimed+1]=target end
...
local function station_pipe_count(pair)
  ...
  local total=0
  if type(_G.tech_priests_inventory_steward_sources_for_pair)=="function" then local ok,sources=pcall(_G.tech_priests_inventory_steward_sources_for_pair,pair); if ok and type(sources)=="table" then for _,src in ipairs(sources) do local inv=src and src.inv; if inv and inv.valid then local ok2,n=pcall(function() return inv.get_item_count(M.pipe_item) end); if ok2 then total=total+(tonumber(n) or 0) end end end end end
  return total
end
local function pipe_unlocked(pair) local c=constraints(); if c and type(c.item_unlocked)=="function" then local ok,unlocked,why=pcall(c.item_unlocked,pair.station.force,M.pipe_item); if ok then return unlocked==true,why end end; return true,"unknown-assumed-unlocked" end
local function request_pipes(pair,plan) local remaining=math.max(1,#(plan.tiles or {})-(tonumber(plan.current_index) or 1)+1); pair.active_supply_request={item=M.pipe_item,count=remaining,source="fluid-output-connection-planner-0696",purpose="construction-output-pipe",fluid=plan.fluid,plan_id=plan.id,tick=now()}; pair.logistic_requested_item={item=M.pipe_item,count=remaining,source="fluid-output-connection-planner-0696",purpose="construction-output-pipe",plan_id=plan.id}; plan.state="waiting-pipe-items"; stat("pipe_item_requests") end
local function clear_pipe_request(pair,plan) for _,field in ipairs({"active_supply_request","logistic_requested_item"}) do local req=pair and pair[field]; if type(req)=="table" and req.source=="fluid-output-connection-planner-0696" and (not plan or req.plan_id==plan.id) then pair[field]=nil end end end
...
local function actionable_input_proposal(pair) for _,proposal in ipairs(pair.fluid_connection_proposals_0689 or {}) do if type(proposal)=="table" and proposal.action=="connect-fluid-input" and proposal.state=="source-network-found" and valid(proposal.machine) and proposal.source and valid(proposal.source.entity) then return true end end; return false end
local function proposal_candidate(pair)
  if actionable_input_proposal(pair) then return nil end
  for _,proposal in ipairs(pair.fluid_output_sink_proposals_0694 or {}) do if type(proposal)=="table" and proposal.action=="connect-fluid-output" and proposal.state=="compatible-sink-found" and proposal.sink and valid(proposal.sink.entity) and valid(proposal.machine) and now()-(tonumber(proposal.tick) or -1000000)<=M.proposal_max_age then return proposal end end
  return nil
end
...
local function create_plan(pair,proposal)
  ...
  local sink_ok,sink_why,sink_state=revalidate_sink(proposal,true); if not sink_ok then record(pair,"output-sink-rejected",sink_why); return nil,sink_why end
  ...
  local tiles=new_tiles(pair,route,plan); if #tiles==0 then return nil,"already-connected-or-existing-route" end; if #tiles>M.max_route_tiles then return nil,"too-many-new-tiles" end
  ...
  root().plans[plan.id]=plan; pair.fluid_output_pipe_plan_0696=plan; pair.fluid_output_pipe_reject_until_0696=nil; stat("output_pipe_plans_created"); stat("output_pipe_tiles_planned",#tiles); record(pair,"output-pipe-plan-created",plan.fluid.." tiles="..tostring(#tiles).." machine="..safe(plan.machine_name).." sink="..safe(plan.sink.entity_name)); return plan,"created"
end
local function ensure_plan(pair)
  local plan=pair.fluid_output_pipe_plan_0696; if type(plan)=="table" and plan.state~="complete" and plan.state~="aborted" then return plan,"existing" end
...
local function validate_plan(pair,plan)
  ...
  local proposal={fluid=plan.fluid,amount_per_craft=plan.amount_per_craft,sink=plan.sink}; local ok,why,state=revalidate_sink(proposal,false); if not ok then return false,why end
  plan.sink_segment_id=state.segment_id
  local output_state=segment_state(plan.machine,plan.output_fluidbox_index,plan.fluid); if not output_state then return false,"machine-output-invalid" end
...
local function complete_plan(pair,plan,reason) release_route(pair,plan); clear_pipe_request(pair,plan); plan.state="complete"; plan.completed_tick=now(); plan.result=reason or "complete"; pair.fluid_output_pipe_plan_last_0696=plan; pair.fluid_output_pipe_plan_0696=nil; local d=doctrine(); if d and type(d.inspect_machine)=="function" and valid(plan.machine) then pcall(d.inspect_machine,pair,plan.machine,true) end; stat("output_pipe_plans_completed"); record(pair,"output-pipe-plan-completed",plan.fluid.." tiles="..tostring(#(plan.tiles or {}))) end
local function abort_plan(pair,plan,reason) release_route(pair,plan); clear_pipe_request(pair,plan); if pair.construction_task_0338 and pair.construction_task_0338.fluid_output_pipe_plan_id_0696==plan.id then pair.construction_task_0338=nil end; plan.state="aborted"; plan.aborted_tick=now(); plan.result=tostring(reason or "aborted"); pair.fluid_output_pipe_plan_last_0696=plan; pair.fluid_output_pipe_plan_0696=nil; pair.fluid_output_pipe_reject_until_0696=now()+M.rejection_cooldown; stat("output_pipe_plans_aborted"); record(pair,"output-pipe-plan-aborted",safe(reason)) end
...
local function after_service(pair,plan,task,acted,why)
  ...
  if task_succeeded(pair,task) or existing_route_pipe(pair,task.target_position,plan) then plan.current_index=index+1; plan.retries[index]=nil; plan.active_tile=nil; stat("output_pipe_tiles_completed"); if not current_tile(plan) then complete_plan(pair,plan,"route-built"); return true,"fluid-output-pipe-plan-complete" end; seed_task(pair,plan); return true,"fluid-output-pipe-tile-complete" end
  if why=="missing-item" then request_pipes(pair,plan); return false,"waiting-output-pipe-items" end
  if why=="blocked" or why=="create-failed" or why=="remove-failed" then plan.retries[index]=(tonumber(plan.retries[index]) or 0)+1; stat("output_pipe_tile_retries"); if plan.retries[index]>=M.max_retries_per_tile then abort_plan(pair,plan,"tile-failed:"..safe(why)..":"..pos_key(task.target_position)); return false,"fluid-output-pipe-plan-aborted" end; return acted,"fluid-output-pipe-tile-retry:"..safe(why) end
  return acted,why
end
...
local function patched_service_pair(pair,reason,...)
  ...
  local plan=pair.fluid_output_pipe_plan_0696; if not plan then plan=select(1,ensure_plan(pair)) end
  if plan then
...
local function patch_build(build) if not (build and type(build.service_pair)=="function") or build.fluid_output_connection_planner_0696_active then return false end; build.fluid_output_connection_planner_0696_active=true; previous_build_service_pair=build.service_pair; build.service_pair=patched_service_pair; return true end
...
function M.install()
  ...
  if log then log("[Tech-Priests 0.1.669] compatible-sink output pipe planner armed; consumed sink interfaces remain valid after route attachment") end
  return true
end
