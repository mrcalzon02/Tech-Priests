-- Tech Priests 0.1.674-dev physical manual rocket-silo item logistics.
--
-- Uses only 0709 readiness reports. Manual silo trash is evacuated first, then
-- missing item ingredients for rocket-part crafting are delivered physically.
-- This module never touches rocket payload inventory, attached cargo, rocket
-- parts, launch flags, transitional requests, recipes, modules, or fluidboxes.
...
local function record(pair, action, detail, force_log)
  ...
  pair.rocket_silo_logistics_last_0710 = event
...
local function claim_target(pair, task)
  ...
  return reservations.claim("machine-logistics", task.target, pair, M.reservation_ttl, {
    ...
    family = task.family,
    item = task.item,
    source = "rocket-silo-logistics-0710",
  })
end
...
local function manual_report(report)
  return report and valid(report.silo)
    and not report.automation_owned
    and not report.launch_sequence_active
end

local function candidate(pair)
  local reports = pair.rocket_silo_reports_0709
  ...
  for _, report in ipairs(reports or {}) do
    if manual_report(report) then
      for _, source in ipairs({
        { inv = report.crafter_trash_inventory, label = "crafter-trash" },
        { inv = report.silo_trash_inventory, label = "silo-trash" },
      }) do
        local entries = contents(source.inv)
        local entry = entries[1]
        if entry then
          local score = dist_sq(pair.priest.position, report.silo.position)
          if not trash or score < trash.score then
            trash = {
              family = "rocket-silo-trash",
              target = report.silo,
              target_name = report.silo_name,
              target_unit = report.silo_unit,
              item = entry.name,
              count = math.min(entry.count, M.max_trash_transfer),
              source_inv = source.inv,
              source_entity = report.silo,
      ...
      if report.state == "manual-input-service-eligible"
        and report.input_inventory and report.input_inventory.valid
      then
        local missing = report.item_ingredients_missing and report.item_ingredients_missing[1]
        if missing and missing.name and missing.missing > 0 then
          local source = source_for_item(pair, missing.name, report.input_inventory)
          local score = dist_sq(pair.priest.position, report.silo.position)
          if not supply or score < supply.score then
            supply = {
              family = "rocket-silo-input",
              target = report.silo,
              target_name = report.silo_name,
              target_unit = report.silo_unit,
              item = missing.name,
              count = math.min(math.max(1, missing.missing), M.max_input_transfer),
              source = source,
              report = report,
              score = score,
            }
...
local function request_move(pair, target, reason)
  ...
  if type(_G.tech_priests_request_movement_0418) == "function" then
    local ok, result = pcall(_G.tech_priests_request_movement_0418, pair, target.position,
      reason or "rocket-silo-logistics-0710", {
        owner = "rocket-silo-logistics-0710",
...
local function sync_custody(pair, task, reason)
  ...
  pair.rocket_silo_custody_0710 = nil
  return false
end
...
local function finish(pair, task, reason)
  ...
  if type(leaf) == "table" and leaf.source == "rocket_silo_logistics_0710" then
...
local function restore_orphan(pair)
  ...
  if pair.rocket_silo_logistics_0710
  ...
  record(pair, "silo-orphan-custody-restored", custody.item .. " x" .. safe(custody.count), true)
  return true
end
...
local function return_to_station(pair, task)
  ...
  if dist_sq(pair.priest.position, pair.station.position) > M.station_reach_sq then
    ...
    request_move(pair, pair.station, "rocket-silo-custody-return-0710")
  ...
  if authority and type(authority.deposit_exact) == "function" then
    ok, why, inserted = authority.deposit_exact(pair, carried.item, carried.count,
      task.family == "rocket-silo-trash" and "rocket-silo-trash-retention" or "rocket-silo-input-return", {})
  end
  ...
  record(pair, "silo-custody-storage-blocked",
...
local function revalidate_target(pair, task)
  ...
  local doctrine = readiness()
  if not (doctrine and type(doctrine.inspect_silo) == "function") then
    return nil, "readiness-unavailable"
  end
  local report = doctrine.inspect_silo(pair, task.target, true)
  task.report = report
  ...
  if task.family == "rocket-silo-input" then
    if report.state ~= "manual-input-service-eligible" then
      return report, "target-not-eligible:" .. safe(report.state)
    end
    local still_missing = false
    for _, missing in ipairs(report.item_ingredients_missing or {}) do
      if missing.name == task.item and (tonumber(missing.missing) or 0) > 0 then
        still_missing = true
        break
      end
    end
    if not still_missing then return report, "ingredient-already-satisfied" end
    if not inv_can_insert(report.input_inventory, task.item, 1) then
      return report, "input-inventory-rejects-item"
    end
  end
  return report, "ready"
end
...
local function begin(pair, selected, reason)
  ...
  pair.rocket_silo_logistics_0710 = task
  if task.family == "rocket-silo-trash" then
    task.phase = "move-to-source"
    request_move(pair, task.target, "rocket-silo-trash-pickup-0710")
    record(pair, "silo-trash-task-began",
      task.item .. " x" .. safe(task.count) .. " from " .. safe(task.target_name))
    return true, "moving-to-silo-trash"
  end
  if not valid(task.source_entity) then
    create_request(pair, task)
    record(pair, "silo-input-waiting-source", task.item .. " -> " .. safe(task.target_name))
    return false, "waiting-source"
  end
  task.phase = "move-to-source"
  request_move(pair, task.source_entity, "rocket-silo-input-source-0710")
  record(pair, "silo-input-task-began",
    task.item .. " x" .. safe(task.count) .. " -> " .. safe(task.target_name))
  return true, "moving-to-input-source"
end
...
local function continue_task(pair, task)
  ...
  if task.phase == "waiting-source" then
    if now() - (tonumber(task.request_tick) or now()) >= M.request_timeout then
      return abort(pair, task, "input-source-timeout")
    end
    ...
    task.phase = "move-to-source"
    request_move(pair, source.entity, "rocket-silo-input-source-ready-0710")
  ...
  if task.phase == "move-to-source" then
    ...
    then
      ...
      return abort(pair, task, "silo-trash-source-empty")
    end
    if dist_sq(pair.priest.position, source.entity.position) > M.pickup_reach_sq then
      request_move(pair, source.entity, "rocket-silo-item-source-0710")
    ...
    record(pair, "silo-item-picked-up",
      task.family .. " " .. task.item .. " x" .. safe(removed))
    if task.family == "rocket-silo-trash" then
      task.phase = "return-custody"
      request_move(pair, pair.station, "rocket-silo-trash-return-0710")
      return true, "returning-silo-trash"
    end
    task.phase = "move-to-target"
    request_move(pair, task.target, "rocket-silo-input-delivery-0710")
    return true, "delivering-silo-input"
  end
  ...
  if task.phase == "move-to-target" then
    ...
    if not report or why ~= "ready" then
      task.phase = "return-custody"
      record(pair, "silo-target-became-ineligible", safe(why))
      return return_to_station(pair, task)
    end
    local carried = task.carried
    if not carried then return abort(pair, task, "custody-missing") end
    local inserted = inv_insert(report.input_inventory, carried.item, carried.count)
    if inserted > 0 then
      carried.count = carried.count - inserted
      stat("input-items-delivered", inserted)
      record(pair, "silo-input-delivered",
        carried.item .. " x" .. safe(inserted) .. " -> " .. safe(task.target_name))
    end
    if carried.count <= 0 then return finish(pair, task, "input-delivered") end
    task.phase = "return-custody"
    sync_custody(pair, task, "input-partial")
    request_move(pair, pair.station, "rocket-silo-input-leftover-return-0710")
    return true, inserted > 0 and "partial-input-delivery" or "input-insert-blocked"
  end
...
local function blocker(pair)
  ...
  local leaf = pair.active_leaf_task_0655
  if type(leaf) == "table" and leaf.source ~= "rocket_silo_logistics_0710"
...
function M.service_pair(pair, reason)
  ...
  restore_orphan(pair)
  local task = pair.rocket_silo_logistics_0710
  ...
  if not selected then return false, "no-silo-task" end
  return begin(pair, selected, reason or "service")
end

local function silo_truth(pair)
  ...
  return {
    ...
    priority = M.move_priority,
    radius = 1.2,
    color = { r = 1, g = 0.4, b = 0.12, a = 0.95 },
    can_move = true,
    source = "rocket_silo_logistics_0710",
  }
end
...
local function patch_leaf_truth()
  ...
  if not (ok and truth and type(truth.truth) == "function")
    or truth.rocket_silo_logistics_0710_wrapped
  then
    return false
  end
  truth.rocket_silo_logistics_0710_wrapped = true
  previous_leaf_truth = truth.truth
  truth.truth = function(pair)
    return previous_leaf_truth(pair) or silo_truth(pair)
  end
  return true
end
...
local function patch_diagnostics()
  ...
  if not (diagnostics and type(diagnostics.pair_dump_lines) == "function")
    or diagnostics.rocket_silo_logistics_0710_wrapped
  then
    return false
  end
  diagnostics.rocket_silo_logistics_0710_wrapped = true
  local previous = diagnostics.pair_dump_lines
  diagnostics.pair_dump_lines = function(...)
    local lines = previous(...)
    lines = type(lines) == "table" and lines or {}
    local r = root()
    lines[#lines + 1] = "PAIR-DUMP-0468 ROCKET-SILO-LOGISTICS-0710 enabled="
      .. safe(r.enabled)
      .. " requests=" .. safe(r.stats["input-item-requests"] or 0)
      .. " pickups=" .. safe(r.stats["silo-item-picked-up"] or 0)
      .. " delivered=" .. safe(r.stats["input-items-delivered"] or 0)
      .. " trash_tasks=" .. safe(r.stats["silo-trash-task-began"] or 0)
      .. " custody_restored=" .. safe(r.stats["silo-orphan-custody-restored"] or 0)
      .. " payload_mutations=0 rocket_parts_mutations=0 launch_flag_mutations=0"
    for _, pair in pairs(pair_map()) do
      if valid_pair(pair) then
        local task = pair.rocket_silo_logistics_0710 or {}
        local custody = pair.rocket_silo_custody_0710 or {}
        lines[#lines + 1] = "PAIR-DUMP-0468 rocket-silo-logistics[" .. safe(station_unit(pair)) .. "]"
...
local function register_service()
  ...
  if broker and type(broker.register_service) == "function" then
    broker.register_service({
      ...
      priority = 57,
      budget = 6,
      note = "physical manual rocket-silo ingredient delivery and trash evacuation",
...
function M.install()
  ...
  patch_diagnostics()
  register_service()
  _G.TechPriestsRocketSiloLogistics0710 = M
  if log then log("[Tech-Priests 0.1.674-dev] physical manual rocket-silo item logistics armed") end
  return true
end
