...
--
-- This module surveys station-local resources and infrastructure roles, chooses a
-- diagnostic next target, and stores pair.master_infrastructure_plan_0644. It does
-- not itself place entities, move priests, or mutate active work. The 0.1.645
...
local function station_count(pair, item)
  local n = 0
  for _, src in ipairs(station_sources(pair)) do if src and src.inv and src.inv.valid then n = n + inv_count(src.inv, item) end end
  return n
end
...
local function scan_resources(pair)
  ...
  local ok, ents = pcall(function() return pair.station.surface.find_entities_filtered({ position = pair.station.position, radius = r, type = "resource" }) end)
  if ok and ents then
    for _, e in pairs(ents) do
      if valid(e) and dist_sq(e.position, pair.station.position) <= r * r and (not e.amount or e.amount > 0) then
        ...
        if not out[name].nearest_d2 or d2 < out[name].nearest_d2 then out[name].nearest_d2 = d2; out[name].nearest = { x = e.position.x, y = e.position.y } end
      end
    end
  end
  return out
...
local function entity_role(e)
  ...
  if e.type == "mining-drill" then if e.name == "tech-priests-emergency-miner" then return "emergency-miner" end return "normal-miner" end
...
local function sorted_resource_names(resources)
  ...
  table.sort(names, function(a, b) local pa = RESOURCE_PRIORITY[a] or 1000; local pb = RESOURCE_PRIORITY[b] or 1000; if pa ~= pb then return pa < pb end return a < b end)
  return names
end

local function has_role(roles, role) return roles and roles.counts and (tonumber(roles.counts[role] or 0) or 0) > 0 end
...
local function summary_counts(resources, roles)
  local res_bits = {}
  for _, name in ipairs(sorted_resource_names(resources)) do local rec = resources[name]; res_bits[#res_bits + 1] = name .. ":" .. tostring(rec.count) end
  local role_bits = {}
...
function M.service_pair(pair, reason)
  ...
  if plan then stat("plans_built"); record("plan-built-0644", pair, "stage=" .. safe(plan.stage) .. " target=" .. safe(plan.target and plan.target.preferred_item) .. " blocker=" .. safe(plan.blocker)); return true, why end
  stat("plans_failed"); return false, why
end

function M.service_all(reason)
  local r = root(); if r.enabled == false then return 0 end
  local n = 0
  for _, pair in pairs(pair_map()) do if n >= M.max_pairs_per_pulse then break end if valid_pair(pair) then local ok = pcall(M.service_pair, pair, reason or "pulse"); if ok then n = n + 1 end end end
...
local function plan_lines(plan)
  ...
  return { "  station=" .. safe(plan.station) .. " radius=" .. safe(plan.radius) .. " stage=" .. safe(plan.stage) .. " status=" .. safe(plan.status), "  resources=" .. safe(plan.resource_summary ~= "" and plan.resource_summary or "none"), "  roles=" .. safe(plan.role_summary ~= "" and plan.role_summary or "none"), "  next class=" .. safe(t.class) .. " preferred=" .. safe(t.preferred_item) .. " fallback=" .. safe(t.fallback_item) .. " resource=" .. safe(t.resource), "  delivery=" .. safe(t.delivery) .. " blocker=" .. safe(t.blocker), "  reason=" .. safe(t.reason) }
end
...
local function install_command()
  ...
  commands.add_command("tp-infra-plan-0644", "Tech Priests 0.1.647: master Martian bootstrap infrastructure plan. Params: status/kick/all/on/off/recent", function(event)
    ...
    local lines = { "[tp-infra-plan-0644] version=" .. M.version .. " enabled=" .. safe(r.enabled) .. " built=" .. safe(r.stats.plans_built or 0) .. " failed=" .. safe(r.stats.plans_failed or 0) }
...
local function install_bootstrap_ghost_planner_0645()
  local ok, Ghost0645 = pcall(require, "scripts.core.construction_bootstrap_ghost_planner_0645")
  if ok and Ghost0645 and type(Ghost0645.install) == "function" then
    local ok2, err2 = pcall(Ghost0645.install)
    if ok2 then stat("bootstrap_ghost_0645_installed"); return true end
    if log then log("[Tech-Priests 0.1.645] construction_bootstrap_ghost_planner_0645 install failed: " .. tostring(err2)) end
    stat("bootstrap_ghost_0645_install_failed")
    return false
  end
  if log then log("[Tech-Priests 0.1.645] construction_bootstrap_ghost_planner_0645 unavailable: " .. tostring(Ghost0645)) end
  stat("bootstrap_ghost_0645_missing")
  return false
end
...
function M.install()
  ...
  install_command()
  install_bootstrap_ghost_planner_0645()
  local broker = rawget(_G, "TechPriestsRuntimeTickBroker0600")
  if broker and type(broker.register_service) == "function" then broker.register_service({ name = "master_infrastructure_plan_0644", category = "diagnostics", interval = M.tick_interval, priority = 980, budget = 8, fn = function(event, budget) M.service_all("broker") return true end, note = "station infrastructure survey plan diagnostics" })
  else
