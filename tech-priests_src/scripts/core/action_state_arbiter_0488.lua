...
local function normalize_kind(v)
 ...
 if k:find("combat%-repair")then return"combat-repair"end;if k:find("combat",1,true)or k:find("defend",1,true)then return"combat"end;if k:find("repair",1,true)then return"repair"end
 if k:find("consecr",1,true)or k:find("sanct",1,true)then return"consecration"end;if k:find("construct",1,true)or k:find("build",1,true)then return"construction"end;if k:find("craft",1,true)or k:find("fabric",1,true)then return"crafting"end
...
local function recommendation(global_name,module_name,pair,expected_kind)
 local mod=rawget(_G,global_name)or package.loaded[module_name];if not(mod and type(mod.recommend_action)=="function")then return nil end;local ok,a=pcall(mod.recommend_action,pair);return ok and type(a)=="table"and a.kind==expected_kind and a or nil
end
local function combat_repair_recommendation(p,order_kind,mode_kind)
 if order_kind~="idle"or not(mode_kind=="combat"or valid(p.combat_target))then return nil end;return recommendation("TechPriestsCombatRepairDoctrine0517","scripts.core.combat_repair_doctrine_0517",p,"combat-repair")
end
local function machine_logistics_recommendation(p,order_kind,mode_kind)
 if order_kind~="idle"or mode_kind=="combat"or valid(p.combat_target)then return nil end;return recommendation("TECH_PRIESTS_MACHINE_LOGISTICS_FULFILLMENT_0528","scripts.core.logistics_machine_fulfillment_0528",p,"machine-logistics")
end
local function item_family_recommendation(p,order_kind)
 if order_kind~="idle"and order_kind~="item-family-logistics"then return nil end;return recommendation("TechPriestsItemFamilyLogistics0702","scripts.core.item_family_logistics_0702",p,"item-family-logistics")
end
