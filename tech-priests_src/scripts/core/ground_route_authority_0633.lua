...
local function record(action, pair, detail, force)
  ...
  if force or now()-last >= M.log_interval then
    r.last_log[key]=now()
    if log then log("[Tech-Priests 0.1.639] "..ev.action.." station="..ev.station.." priest="..ev.priest.." "..safe(detail)) end
  end
  return ev
end
...
local function install_command()
  ...
  commands.add_command("tp-ground-route-0633", "Tech Priests 0.1.639: Ground Route Authority diagnostics. Params: on/off/chunk-on/chunk-off/status", function(event)
...
local function install_0634_0635_0637_0638_0639_repairs()
...
function M.install()
  M.root()
  M.wrap_request()
  install_0634_0635_0637_0638_0639_repairs()
  install_command()
  _G.TechPriestsGroundRouteAuthority0633 = M
  if log then log("[Tech-Priests 0.1.639] Ground Route Authority installed; movement requests now receive route leases, bounded visible waypoints, inventory-deposit crash safety, and stale supply-writ satisfaction clearing") end
  return true
end
