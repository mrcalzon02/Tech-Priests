...
local function record(action, pair, detail, force)
  ...
  if force or now()-last >= M.log_interval then
    r.last_log[key]=now()
    if log then log("[Tech-Priests 0.1.643] "..ev.action.." station="..ev.station.." priest="..ev.priest.." "..safe(detail)) end
  end
  return ev
end
...
local function install_command()
  ...
  commands.add_command("tp-ground-route-0633", "Tech Priests 0.1.643: Ground Route Authority diagnostics. Params: on/off/chunk-on/chunk-off/status", function(event)
...
local function install_0634_0635_0637_0638_0639_0640_0642_0643_repairs()
  ...
  local ok_placement, Placement0643 = pcall(require, "scripts.core.emergency_facility_placement_bridge_0643")
  if ok_placement and Placement0643 and type(Placement0643.install)=="function" then pcall(Placement0643.install) end
...
function M.install()
  M.root()
  M.wrap_request()
  install_0634_0635_0637_0638_0639_0640_0642_0643_repairs()
  install_command()
  _G.TechPriestsGroundRouteAuthority0633 = M
  if log then log("[Tech-Priests 0.1.643] Ground Route Authority installed; movement leases, deposit safety, stale supply clearing, infrastructure-first gating, emergency facility placement bridge, and behavior-tree monitoring active") end
  return true
end
