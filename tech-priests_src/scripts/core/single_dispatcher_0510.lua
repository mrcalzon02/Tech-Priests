...
local OWNERSHIP = {
  ...
  ["artillery-logistics"] = "dispatcher_owns_artillery_logistics",
  ["roboport-repair-pack-logistics"] = "dispatcher_owns_roboport_repair_pack_logistics",
}
...
local function root()
  ...
  for _, key in pairs(OWNERSHIP) do
    if state[key] == nil then state[key] = true end
  end
  if state.enabled == nil then state.enabled = true end
  if state.gate_legacy_tick == nil then state.gate_legacy_tick = true end
  if state.suppress_independent_executor_pulses == nil then
    state.suppress_independent_executor_pulses = true
  end
...
local function family_for(pair, action)
  ...
  if kind == "item-family-logistics" or kind == "item_family_logistics" then return "item-family-logistics" end
  if kind == "energy-family-logistics" or kind == "energy_family_logistics" then return "energy-family-logistics" end
  if kind == "rocket-silo-logistics" or kind == "rocket_silo_logistics" then return "rocket-silo-logistics" end
  if kind == "artillery-logistics" or kind == "artillery_logistics" then return "artillery-logistics" end
  if kind == "roboport-repair-pack-logistics" or kind == "roboport_repair_pack_logistics" then
    return "roboport-repair-pack-logistics"
  end
...
local function publish_action(pair, action, family, status, detail)
  ...
  local order = pair.order_queue_0469 and pair.order_queue_0469.current or pair.active_order_0469
  ...
  local record_data = {
    ...
    issued_tick = previous and previous.action_id == action_id and previous.issued_tick or now(),
    updated_tick = now(),
  }
  pair.canonical_action_0744 = record_data
  pair.dispatcher_0510 = {
    tick = record_data.updated_tick,
    action = safe(action and action.kind or family),
    family = family,
    reason = safe(action and action.reason or action and action.source or detail),
    target = safe(record_data.target_name
      and (record_data.target_name .. "#" .. safe(record_data.target_unit)) or "nil"),
    gates_legacy = OWNERSHIP[family] ~= nil and status ~= "idle" and status ~= "failed",
    acted = status == "acted",
    result = safe(detail),
  }
  return record_data
...
local function call_module(module_name, global_name, function_name, pair, reason)
  ...
  if not (module and type(module[function_name]) == "function") then
    return { processed = 1, failed = 1, detail = module_name .. "." .. function_name .. " unavailable" }
  end
...
local EXECUTORS = {
  ["direct-acquisition"] = { "scripts.core.direct_acquisition_executor_0513", "TechPriestsDirectAcquisitionExecutor0513" },
  ["consecration"] = { "scripts.core.consecration_executor_0515", "TechPriestsConsecrationExecutor0515" },
  ["combat-repair"] = { "scripts.core.combat_repair_doctrine_0517", "TechPriestsCombatRepairDoctrine0517" },
  ...
  ["energy-family-logistics"] = { "scripts.core.energy_family_logistics_0707", "TechPriestsEnergyFamilyLogistics0707" },
  ["rocket-silo-logistics"] = { "scripts.core.rocket_silo_logistics_0710", "TechPriestsRocketSiloLogistics0710" },
  ["artillery-logistics"] = { "scripts.core.artillery_logistics_0713", "TechPriestsArtilleryLogistics0713" },
  ["roboport-repair-pack-logistics"] = { "scripts.core.roboport_repair_pack_logistics_0715", "TechPriestsRoboportRepairPackLogistics0715" },
}
...
local function execute(pair, family, reason)
  if family == "station-craft" then
    ...
    if result.failed == 0 and (result.acted > 0 or result.waiting > 0 or result.blocked > 0) then
      return result
    end
  ...
  if spec then return call_module(spec[1], spec[2], "service_pair", pair, reason) end
  return {
    processed = 1,
...
function M.service_pair(pair, reason)
  ...
  if type(_G.tech_priests_0507_action_claim) == "function" then
    pcall(
      _G.tech_priests_0507_action_claim,
      pair,
      family,
      "single-dispatcher_0510",
      action.reason or reason or "service"
    )
  end
  ...
  local status = execution.failed > 0 and "failed"
    ...
    or execution.waiting > 0 and "waiting"
    or "idle"
...
function M.service_all(reason, budget)
  ...
  local limit = math.max(1, math.min(#list, math.floor(tonumber(budget) or M.max_pairs_per_pulse)))
...
function M.should_gate_legacy(pair)
  ...
  return dispatcher
    and now() - (tonumber(dispatcher.tick) or -1000000) <= M.legacy_gate_window
    and dispatcher.gates_legacy == true
end
...
local function wrap_executor_pulses()
  ...
  then
    ...
    acquisition.pulse = function(reason)
      ...
      if state.enabled ~= false and state.suppress_independent_executor_pulses ~= false
        and not state.dispatching and not text:find("manual", 1, true)
        and not text:find("kick", 1, true) and not text:find("dispatcher%-0510")
      then
...
local function wrap_diagnostics()
  ...
  diagnostics.pair_dump_lines = function(...)
    ...
    local state = root()
    lines[#lines + 1] = "PAIR-DUMP-0468 SINGLE-DISPATCHER-0510 version=" .. M.version
      .. " processed=" .. safe(state.stats.pairs_processed or 0)
...
function M.install()
  ...
  wrap_diagnostics()
  if commands and commands.remove_command then pcall(commands.remove_command, "tp-dispatcher-0510") end
  ...
  local service = broker.register_service({
    ...
    fn = function(_, budget_value) return M.service_all("broker-0510", budget_value) end,
  })
  _G.TechPriestsSingleDispatcher0510 = M
  if log then
    log("[Tech-Priests recovery] canonical dispatcher installed; artillery and roboport aware")
  end
  return service ~= nil
end
