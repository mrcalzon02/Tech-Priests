...
function M.item_for_entity(entity_name)
  ...
  for item_name, item in pairs(prototypes.item) do
    local place_result
    pcall(function() place_result = item.place_result end)
    if place_result and place_result.name == entity_name then
...
function M.item_unlocked(force, item_name)
  ...
  if unlock_cache_tick ~= tick then
    unlock_cache_tick = tick
    unlock_cache = {}
  end
  local key = tostring(force.index or force.name or "?") .. ":" .. item_name
  if unlock_cache[key] ~= nil then
    return unlock_cache[key], unlock_cache[key]
      and "enabled-recipe" or "technology-locked-or-no-enabled-recipe"
  end
...
function M.entity_unlocked(pair, entity_name)
  ...
  local unlocked, reason = M.item_unlocked(pair.station.force, item_name)
  return unlocked, reason, item_name
end
...
M.HARDENERS = HARDENERS
M.RETIRED = RETIRED
M.FAMILY_TARGETS = FAMILY_TARGETS
...
local function disable_target(module_name, family, reason)
  ...
  if not (ok and module) then return false end
  local disabled = false
  if type(module.root) == "function" then
    local ok_root, state = pcall(module.root)
  ...
  module.recovery_degraded_0744 = true
  module.recovery_degraded_reason_0744 = tostring(reason)
  return disabled
end
...
local function degrade_failure(failure)
  ...
  local record = M.degraded_families[family]
  ...
  for _, module_name in ipairs(FAMILY_TARGETS[family] or {}) do
    if disable_target(module_name, family, failure.label .. ":" .. failure.reason) then
      record.targets_disabled = record.targets_disabled + 1
    end
  end
  return family
end
...
local function ensure_broker(phase)
  ...
  if not broker then
    ...
    if not ok then return false, "broker-require-failed:" .. safe(loaded) end
    broker = loaded
  end
  if not (broker and type(broker.install) == "function") then
    return false, "broker-install-unavailable"
  end
  local ok, result = pcall(broker.install)
  if not (ok and result == true) then
    return false, ok and ("broker-install-return:" .. safe(result))
      or ("broker-install-error:" .. safe(result))
  end
  local summary = type(broker.installation_summary) == "function"
    and broker.installation_summary() or nil
  if not (summary and summary.complete == true and summary.installed == true
    and summary.route_registered == true
    and summary.route_id == "runtime_tick_broker_0600:central-pulse")
  then
    return false, "broker-route-incomplete"
  end
  M.broker_prearm = {
    phase = phase,
    complete = true,
    reason = "canonical-broker-ready",
    route_id = summary.route_id,
  }
  return true, "canonical-broker-ready"
end

local function install_hardener(module_name)
  local ok, module = pcall(require, module_name)
  if not (ok and module and type(module.install) == "function") then
    return false, ok and "module missing install()" or tostring(module)
  end
  local ok_install, result = pcall(module.install)
  if not ok_install then return false, tostring(result) end
  if result ~= true then
    return false, "install must return literal true; received " .. tostring(result)
  end
...
local function attempt_all(phase)
  ...
  for _, spec in ipairs(HARDENERS) do
    snapshot.attempted = snapshot.attempted + 1
    local ok, reason = install_hardener(spec.module)
    snapshot.results[spec.label] = {
      module = spec.module,
      ok = ok == true,
  ...
  snapshot.complete = #snapshot.failures == 0
  return snapshot
end
...
local function broker_failure_snapshot(phase, reason)
  return {
    ...
    failures = {
      {
        module = "scripts.core.runtime_tick_broker",
        label = "runtime_tick_broker_0600",
...
function M.finalize_installation(reason)
  ...
  local broker_ok, broker_reason = ensure_broker("post-loader")
  local snapshot = broker_ok and attempt_all("post-loader")
    or broker_failure_snapshot("post-loader", broker_reason)
  ...
  if log then
    log("[Tech-Priests recovery] final hardener installation attempted="
      ...
      .. " broker=" .. safe(M.broker_prearm.reason))
  end
...
function M.feature_available(family)
  return M.install_complete == true
    or M.degraded_families[tostring(family or "runtime")] == nil
end

function M.install()
  _G.TechPriestsPlanningConstraints0646 = M
  local broker_ok, broker_reason = ensure_broker("prearm")
  if not broker_ok then
    M.install_phase = "broker-unavailable"
    M.broker_prearm = {
      phase = "prearm",
      complete = false,
      reason = tostring(broker_reason),
    }
    M.prearm_results = {}
    M.prearm_failures = {
      {
