...
-- Commandless movement target reconciler.
...
local function request_owner(req) return lower((req and (req.owner or req.reason)) or "") end
...
local function request_points_to_lock(req, lock) return req and lock and lock.position and same_pos(req, lock.position, 0.55) end

local function make_lock_request(pair, lock, reason)
  local req = {
    x = lock.position.x, y = lock.position.y, radius = M.request_radius,
    reason = reason or "direct-target-reconciler-0652", owner = "direct-target-reconciler-0652",
    priority = 940, distraction = defines and defines.distraction and defines.distraction.none or nil,
    issued_tick = now(), updated_tick = now(), expires_tick = now() + M.request_ttl,
    last_command_tick = 0, last_distance_sq = nil, direct_target_lock_0652 = true,
    target_name = lock.name or (valid(lock.entity) and lock.entity.name) or nil,
    target_unit = valid(lock.entity) and lock.entity.unit_number or nil, item = lock.item,
  }
  local mr = movement_root(); local key = pair_key(pair)
  if key then mr.requests[key] = req; mr.active_request_ids[key] = true end
...
function M.reconcile_pair(pair, reason)
  ...
  pair.direct_target_reconciler_0652 = { tick = now(), item = lock.item, target = lock.name or lock.entity.name, x = lock.position.x, y = lock.position.y, previous_request_owner = current_owner, previous_request = current_target, reason = reason or "reconcile" }
  if request_points_to_lock(current, lock) and request_is_direct(current) then current.updated_tick = now(); current.expires_tick = now() + M.request_ttl; return false, "already-direct-lock-request" end
...
local function wrap_movement_request()
  ...
  _G.tech_priests_request_movement_0418 = function(pair, destination, reason, opts, ...)
    ...
    if lock and not request_points_to_lock(destination, lock) then
      ...
      if not exempt and not directish then M.reconcile_pair(pair, "request-wrapper-redirect-0652"); return true, pair and pair.movement_request_0418 or nil end
    end
...
function M.service_all(reason)
  ...
  for _, pair in pairs(pair_map()) do
    ...
    if valid_pair(pair) then local ok, acted = pcall(M.reconcile_pair, pair, reason or "pulse"); if ok and acted then n = n + 1 end end
  end
  return n
end

function M.install()
  root(); wrap_movement_request(); _G.TechPriestsMovementTargetReconciler0652 = M
  ...
  if log then log("[Tech-Priests 0.1.653] movement target reconciler installed") end
  return true
end
