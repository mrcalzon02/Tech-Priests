...
def validate_stage1(t: dict[str, str], errors: list[str]) -> None:
    ...
    start = t["emergency"].find("local function collect_facility_output")
    end = t["emergency"].find("\nlocal function ", start + 1)
    collection = t["emergency"][start : end if end > start else None]
    if start < 0 or "assembling_machine_input" in collection:
        errors.append("emergency facility output collection is missing or reads input inventory")
    ...
    require("order", t["order"], [
        ...
        "clear_target", "promote(p,q,why)", "run_callback==false",
        'return n>0,"acted="..n', "r.cursor",
    ], errors)
    forbid("order", t["order"], ['activate(p,q,o,"submit")', 'activate(p,q,o,"preempt")'], errors)
...
def validate_artifacts(t: dict[str, str], errors: list[str]) -> None:
    ...
    expected = {
        "release": "v0.1.674-rc.3", "version": "0.1.674",
